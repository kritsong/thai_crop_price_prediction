...
OUT = str(IMAGE_DIR / "typical_qualitative_predictions.png")

# (product_id, English label, English group name), ordered by group size
# (largest active-series group first), matching Table 1 of the paper.
PRODUCTS = [
    ("P13090", "Thai water spinach (10 bundles), retail", "Fresh Vegetables"),
    ("W11005", "Dressed pork, general grade, wholesale", "Meat & Poultry"),
    ("W18034", "Refined palm oil, bottled 1L, wholesale", "Oil Crops & Oils"),
    ("P15040", "Dried chili (Yot Son), secondary grade, retail", "Food Crops"),
    ("W14025", "Nam Dok Mai mango grade 1, wholesale", "Fruits"),
    ("R11017", "Broken white rice C1, wholesale", "Wholesale Rice & Bags"),
    ("W16042", "Feed corn kernels, Phetchabun, wholesale", "Field Crops"),
    ("P12001", "White shrimp (40 pcs/kg), retail", "Aquatic & Marine"),
    ("W17004", "Fish meal, protein <=60%, wholesale", "Feed & Raw Materials"),
]
...
def main():
    ...
    for ax, (pid, label, group) in zip(axes.ravel(), PRODUCTS):
        ...
        ax.plot(pd.to_datetime(r["lookback_dates"]), r["lookback_actual"],
                 color="#222222", linewidth=1.4, label="Lookback (actual)")
        ax.plot(pd.to_datetime(r["future_dates"]), r["future_actual"],
                 color="#2166ac", linewidth=1.4, label="Held-out (actual)")
        ax.plot(pd.to_datetime(r["future_dates"]), r["median"],
                 color="#b2182b", linewidth=1.2, linestyle="--", label="Median forecast")
        ax.fill_between(pd.to_datetime(r["future_dates"]), r["lower"], r["upper"],
                         color="#b2182b", alpha=0.15, label="80% interval")
        ax.axvline(pd.to_datetime(r["future_dates"][0]), color="#555555", linewidth=0.7, linestyle=":")
        ax.set_title(f"{pid}: {label}\n({group})", loc="left", fontsize=8.3, fontweight="bold")
        ax.set_ylabel("Price (THB)", fontsize=8.5)
    ...
    fig.suptitle("Median-representative forecasts, one per commodity group (real inference)",
